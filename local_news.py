...
def scrape_theinformant247():
    ...
    for container in containers:
        ...
        address = container.find(class_="post-thumb").attrs['href']
        read_address = requests.get(address)
        page_html = read_address.text
        page_soup = soup(page_html, "html.parser")
...
def scrape_freshinsight():
    ...
    for container in containers:
        # headline = container.div.h3.a.get_text().strip()
        headline = container.find(class_="rcp-title").get_text().strip()
        image = container.find(class_="rcp-title").a["style"]
        date = container.find(class_="featured-date").get_text().strip()
        author = container.find(class_="featured-author idel").get_text().strip()
