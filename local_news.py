...
def scrape_legit():
    ...
    for container in containers:
        x = \
            container.find_all(class_='c-article-card-no-border__headline-hover-inner'
                               )
        for y in x:
    ...
    #         news_snip = container.find(class_="tmpost-desc").get_text()
        ...
        for y in x:
            ...
            x = \
                page_soup.body.find_all(class_='l-article__body c-article__body'
                    )
            for y in x:
                post = str(y)
                ...
                news_read = soup(post[cut_from + 1:cut_to],
                                 'html.parser')
                print news_read
