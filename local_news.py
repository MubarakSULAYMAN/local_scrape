...
page_html = newsClient.text# newsClient.close()
page_soup = soup(page_html, "html.parser")
containers = page_soup.findAll("div", {
  "class": "l-taxonomy-page-hero__list"
})
...
x = container.find_all(class_ = "c-article-card-no-border")
for y in x:
  image = y.a.img['src']# print(image)# news_snip = container.find(class_ = "tmpost-desc").get_text()

x = container.find_all(class_ = "c-article-card-no-border")
for y in x:
  fetch_read_address = y.a["href"]# print(fetch_read_address)
read_address = requests.get(fetch_read_address)# print(read_address)

page_soup = soup(read_address.content, "html.parser")# print(page_soup)

newspage = page_soup.body.find_all(class_ = "c-article-info")
x = newspage.find_all(class_ = 'c-article-info__author')
for y in x:
  author = y.getText().strip()# print(author)
...
newsClient = requests.get(news_url)# page_html = newsClient.text# page_soup = soup(page_html, "html.parser")# containers = page_soup.findAll("div", {
...
#
for container in containers: #headline = container.div.p.getText().strip()# address = container.h1.a["href"]# author = container.find(class_ = "author").getText().strip()# date = container.find(class_ = "entry-date").getText().strip()# image = container.find(class_ = "content-excerpt").a.img["src"]

# fetch_read_address = container.find(class_ = "entry").a['href']# read_address = requests.get(fetch_read_address)# page_soup = soup(read_address.content, "html.parser")# post = str(page_soup.body.find(class_ = "post"))# cut_from = post.find('>')# cut_to = post.find('<div class=')# news_read = soup(post[cut_from + 1: cut_to], "html.parser")
...
news_read = soup(post[cut_from + 1: cut_to], "html.parser")# print(news_read)

# newspage = page_soup.body.find_all(class_ = "td-container")# x = newspage.find_all(class_ = "td-post-author-name")# for y in x: #author = y.getText().strip()# print(author)

# newspage = page_soup.body.find_all(class_ = "td-container")# x = fetch_read_address.find(class_ = 'td-post-date')# for y in x: #date = y.getText().strip()# print(date)
...
for y in x:
  headline = y.a.getText().strip()# print(headline)
...
for y in x:
  image = y.img['src']# print(image)# news_snip = container.find(class_ = "tmpost-desc").get_text()
x = container.find_all(class_ = "mvp-main-blog-text")
for y in x:
  fetch_read_address = y.a["href"]# print(fetch_read_address)
...
# def scrape_theinformant247(): #news_url = "http://www.theinformant247.com"#
source = "Informant 247"#
newsClient = requests.get(news_url)# page_html = newsClient.text# newsClient.close()# page_soup = soup(page_html, "html.parser")# containers = page_soup.findAll("div", {
  "class": "container-wrapper"
})
...
#
for container in containers: #headline = container.div.a["title"]# address = container.div.a["href"]# author = "The Informant247"#
date = container.find(class_ = "time").getText().strip()# image = container.find(class_ = "attachment-jannah-image-large size-jannah-image-large wp-post-image").a.img['src']# fetch_read_address = container.find(class_ = "post-details").a["href"]# read_address = requests.get(fetch_read_address)# page_soup = soup(read_address.content, "html.parser")# post = str(page_soup.body.find(class_ = "entry-content"))# cut_from = post.find('</div>')# cut_to = post.find("<div")# news_read = soup(post[cut_from + 1: cut_to], "html.parser")
...
for y in x:
  image = y.img["src"]# print(image)# news_snip = container.find(class_ = "post-title").a["href"]
...
# def scrape_royalfm(): #news_url = "http://royalfm.net/category/news/local_news/"#
source = "Royal fm - 95.1 MHz"#
newsClient = requests.get(news_url)# page_html = newsClient.text# newsClient.close()# page_soup = soup(page_html, "html.parser")# containers = page_soup.findAll("div", {
  "class": "category-local_news"
})

# royal_news = []

#
for container in containers: #headline = container.div.a["title"]# address = container.div.a["href"]# author = container.find(class_ = 'tm_catpost_item_1').getText().strip()# date = container.find(class_ = 'tm_catpost_item_3').getText().strip()# image = container.find(class_ = 'tm_cat_image').a.img['src']
